...
const DigitalReceipt = ({ order, onExit, theme }: { order: Order, onExit: () => void, theme: string }) => {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900 z-[100] flex flex-col items-center font-mono overflow-hidden">
            <div className="flex-1 w-full overflow-y-auto py-10 px-4 flex flex-col items-center custom-scrollbar">
                ...
                <div className="w-full max-w-sm flex flex-col gap-3 shrink-0 pb-10">
                    ...
                    <button onClick={handleCloseAttempt} className="w-full bg-orange-600 hover:bg-orange-700 text-white font-black py-4 rounded-2xl shadow-2xl transition-all active:scale-95">SELESAI</button>
                </div>
            </div>
        </div>
    );
...
const CustomerOrderView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    useEffect(() => {
        ...
        if (q) {
            try {
                // Dekripsi base64 dari QR (Format: B:branch|T:tableNum)
    ...
    if (!isStoreOpen) {
        return (
            <div className="min-h-[100dvh] bg-orange-50 flex flex-col items-center justify-center p-8 text-center">
                <h2 className="text-3xl font-black mb-4">KEDAI TUTUP</h2>
    ...
    return (
        <div className="flex flex-col h-[100dvh] bg-orange-50 overflow-hidden">
            <header className="bg-white border-b px-4 py-4 flex items-center gap-3 sticky top-0 z-20">
                ...
                <h1 className="font-black text-xl uppercase italic">Bakso Ujo</h1>
            </header>

            <div className="bg-white px-4 py-2 border-b flex gap-2 overflow-x-auto whitespace-nowrap no-scrollbar">
                <button onClick={() => setActiveCategory('All')} className={`px-5 py-2 rounded-full text-xs font-black ${activeCategory === 'All' ? `bg-orange-600 text-white` : 'bg-gray-100 text-gray-500'}`}>SEMUA</button>
                {categories.map(cat => <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-5 py-2 rounded-full text-xs font-black ${activeCategory === cat ? `bg-orange-600 text-white` : 'bg-gray-100 text-gray-500'}`}>{cat.toUpperCase()}</button>)}
            </div>
            ...
            {cartCount > 0 && !isCartOpen && (
                <div className="fixed bottom-6 left-6 right-6 z-40">
                    <button onClick={() => setIsCartOpen(true)} className="w-full bg-orange-600 text-white p-4 rounded-2xl shadow-xl flex justify-between items-center ring-4 ring-white">
                        <div className="flex items-center gap-3"><span className="bg-white/20 px-2 py-1 rounded-lg font-black">{cartCount}</span><span className="font-black">{formatRupiah(cartTotal)}</span></div>
                ...
                <div className="fixed inset-0 bg-black/60 z-[60] flex flex-col justify-end animate-fade-in">
                    <div className="bg-white rounded-t-[2.5rem] max-h-[90vh] flex flex-col animate-slide-in-up">
                        <div className="p-6 border-b flex justify-between items-center">
                            <h2 className="text-2xl font-black uppercase italic">Keranjang Belanja</h2>
                            <button onClick={() => setIsCartOpen(false)} className="bg-gray-100 p-2 rounded-full">&times;</button>
                        </div>
                            ...
                            {cart.map(item => (
                                <div key={item.id} className="bg-gray-50 p-4 rounded-2xl flex justify-between items-center">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-orange-600 font-bold">{formatRupiah(item.price)}</p>
                                    </div>
                                    <div className="flex items-center gap-3 bg-white p-1 rounded-xl">
                                        <button onClick={() => updateQty(item.id, -1)} className="w-8 h-8 font-black text-red-500">-</button>
                                        <span className="font-black">{item.quantity}</span>
                                        <button onClick={() => updateQty(item.id, 1)} className="w-8 h-8 font-black text-green-600">+</button>
                                    </div>
                                </div>
                            ))}
                            <div className="space-y-4 mt-6">
                                <input value={customerName} onChange={e => setCustomerName(e.target.value)} className="w-full bg-gray-100 p-4 rounded-xl font-bold" placeholder="Nama Anda" />
                                <input value={tableNumber} onChange={e => setTableNumber(e.target.value)} type="text" className="w-full bg-gray-100 p-4 rounded-xl font-bold" placeholder="Nomor Meja" />
                            </div>
                        </div>
                        <div className="p-6 bg-white border-t">
                            ...
                            <button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-orange-600 text-white font-black py-5 rounded-[1.5rem] shadow-xl uppercase">
