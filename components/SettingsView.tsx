...
const SettingsView: React.FC = () => {
    ...
    const [activeTab, setActiveTab] = useState<'profile' | 'menu' | 'tables' | 'staff' | 'operational'>('profile');
    
    // --- STATE FOR OPERATIONAL ---
    ...
    const handleAddMotivation = () => {
        if (newMotivation.trim()) {
            setStoreProfile({
                ...storeProfile,
                kitchenMotivations: [...storeProfile.kitchenMotivations, newMotivation.trim()]
            });
            setNewMotivation('');
        }
    };
    ...
    const handleRemoveMotivation = (index: number) => {
        const updated = storeProfile.kitchenMotivations.filter((_, i) => i !== index);
        setStoreProfile({ ...storeProfile, kitchenMotivations: updated });
    };
        ...
        <div className="flex flex-col h-full bg-gray-50 overflow-hidden font-sans">
            ...
            <div className="bg-white border-b px-8 py-4 flex flex-col lg:flex-row items-center justify-between sticky top-0 z-20 gap-4">
                ...
                <div className="flex bg-gray-100 p-1 rounded-xl w-full lg:w-auto overflow-x-auto scrollbar-hide">
                    ...
                    <button onClick={() => setActiveTab('operational')} className={`flex-1 lg:flex-none px-6 py-2 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${activeTab === 'operational' ? 'bg-white shadow text-orange-600' : 'text-gray-500'}`}>OPERASIONAL</button>
                        ...
                        <div className="animate-fade-in space-y-8">
                            <div className="bg-white rounded-3xl shadow-sm border p-8">
                                ...
                                <div className="max-w-md">
                                    <label className="block text-xs font-bold text-gray-400 uppercase mb-2">Alarm Pesanan Terlambat (Menit)</label>
                                    <div className="flex items-center gap-4">
                                        <input 
                                            type="number" 
                                            value={Math.floor(storeProfile.kitchenAlarmTime / 60)} 
                                            onChange={e => setStoreProfile({...storeProfile, kitchenAlarmTime: (parseInt(e.target.value) || 1) * 60})} 
                                            className="w-24 border-2 border-gray-100 rounded-xl p-3 focus:border-red-600 outline-none font-black text-center text-xl" 
                                        />
                                        <div className="text-sm text-gray-500 font-medium">
                                            Pesanan akan berkedip merah jika belum selesai dalam waktu ini.
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex gap-2">
                                        <input 
                                            ...
                                            placeholder="Tulis kata penyemangat..." 
                                            className="flex-1 border-2 border-gray-100 rounded-xl p-3 outline-none focus:border-indigo-600" 
                                        />
                                        <button onClick={handleAddMotivation} className="bg-indigo-600 text-white font-bold px-6 rounded-xl hover:bg-indigo-700 transition-all shadow-md">+</button>
                                    </div>
                                    <div className="space-y-2">
                                        {storeProfile.kitchenMotivations.map((text, idx) => (
                                            <div key={idx} className="flex items-center justify-between bg-gray-50 p-4 rounded-xl border border-gray-100 group">
                                                <span className="text-sm font-bold text-gray-700 italic">"{text}"</span>
                                                <button onClick={() => handleRemoveMotivation(idx)} className="text-red-400 hover:text-red-600 p-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <div className="flex justify-start">
                                <button onClick={handleSaveProfile} className="bg-gray-900 text-white font-black px-10 py-4 rounded-2xl hover:bg-black shadow-xl transition-all active:scale-95 uppercase tracking-widest text-sm">Simpan Pengaturan Operasional</button>
                            </div>
                        </div>
                    )}
